...
1. Paper
    - 화면 위에 떠있는 _카드_ 나 종이를 의미한다.
    - `elevation={3}` 속성 등으로 그림자 깊이 조절이 가능하다.

2. Box
    - `<div>`와 거의 유사한 역할인데, MUI 기반 css를 JS 객체 형태로 넣을 수 있다는 특징이 있다.
    - 이상의 코드에서는 다수의 Typography 컴포넌트들을 통째로 묶어서 마진 등을 조절할 때 사용했다.

3. Avatar
    - 사용자 프로필 사진이나 이름 첫 문자를 동그랗게 띄워줄 때 사용한다.
    - 이상의 코드에서는 그 사람 모양 아이콘 집어넣으려고 사용했다.
  ...
  return (
    <>
      <Container maxWidth='xs'>
        <Box sx={{mt: s, display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
...
```
와 같이 가지고 올 수 있도록 작성하셨다. 즉, google 로그인이 성공한다면
`http://localhost:5173/oauth2/callback?token=eyj...&email=kim1@gmail.com&name=김일&role=ROLE_USER`라는 전체 url을 return하게 되는데, 그 중에서 `?`뒷 부분에 있는 부분이 key-value형태와 유사하다는 것을 알 수 있다. token, email, name, role이라는 key를 갖고, `=` 뒷 부분이 value가 되며, 다음 key로 연결될 때 `&`를 쓴 형태다. 백엔드에서는 `@PathVariable`을 통해서 endpoint 뒷부분의 id를 동적으로 가져올 수 있었다.
`http://localhost:8080/api/vehicles/1`에서 `/1`부분.
그런데 search 라고 하는 엔드포인트를 만들었던적도 있다. repository 내에서 커스텀 메서드를 만들었을 때, 그때도 물음표 부분을 확인할 수 있었는데 이처럼 `?` 뒷 부분을 `쿼리스트링(query string)`이라고 칭한다.

- 그렇다면 searchParams가 무슨 역할을 하는가? -> 현재 브라우저 주소창에 있는 쿼리 스트링을 읽어오거나 수정할 때 사용한다.

- 즉 `const [ searchParams ] = useSearchParams();`라고 쓴 다음에 `searchParams.get('키이름');`을 하면, 예를 들어 key가 token 이라면 `http://localhost:5173/oauth2/callback?token=eyj...&email=kim1@gmail.com&name=김일&role=ROLE_USER` 여기서 token=뒷부분부터 & 앞까지의 값을 string으로 가져온다는 의미가 된다.

- `??` 란 ? (null 병합 연산자)
- JS에서 null 오류를 잡기 위한 것인데, `??` 왼쪽 값이 null 이면 오른쪽 값으로 대입한다는 것이다.
`email : eamil ?? ''`을 썼다면 email key에 email value를 대입할 때 null/undefined의 경우에 공백('')을 입력하여 아예 email key가 존재하지 않아서 생겨나는 오류를 방지하는 역할이다.

- 그렇다면 `||`와 차이가 없어 보일탠데
    - 차이점 : `||`dms `0, false, ''`를 전부 다 null 값 처리해버리기 때문에 0 || '' 이라고 쓰면 ''가 입력될 것이다.
    - `??`는 오직 null/undefined만 ?? 오른쪽 값으로 처리해준다.

- `{replace: true}` : OAuth2 로그인이 완료된 다음에 `/`를 통해 메인페이지로 진입하게 되는데, 그때 뒤로가기 버튼을 누르면 다시 callback page가 나오게 될 것이다. 그러면 불필요한 fetch가 일어날 수 있기 때문에 그것을 막아준다.
