...
// 요청 interceptor - 특정 요청들이 들어왔을 때 사전에 가로챈 다음에 그 다음 작업이 수행될 수 있도록 함.
...
export const updateUserApi = async (data: LoginRequest) => {
  const res = await api.put('/api/auth/user', data);
  return res.data;
};
export default api;